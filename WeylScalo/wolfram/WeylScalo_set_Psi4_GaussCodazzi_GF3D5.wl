...
SetOutputFile[FileNameJoin[{Directory[], "WeylScalo_set_Psi4_GaussCodazzi_GF3D5.hxx"}]];
...
SetMainPrint[
  ...
  pr[];
  PrintEquations[{Mode -> "Temp"}, GaussCodazziVarlist];
  pr[];
  PrintEquations[{Mode -> "Temp"}, TetradVarlist];
  pr[];
  PrintEquations[{Mode -> "Main", SuffixName -> "GaussCodazzi"}, Psi4Varlist];
  pr[];
  pr["  });"];
  pr["});"];
];
